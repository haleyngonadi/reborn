...
function vp_pfui_base_url() {
	return trailingslashit(apply_filters('vp_pfui_base_url', get_template_directory_uri('', __FILE__)));
}
...
function vp_pfui_add_meta_boxes($post_type) {
	if (post_type_supports($post_type, 'post-formats') && current_theme_supports('post-formats')) {
		// assets
		wp_enqueue_script('vp-post-formats-ui', vp_pfui_base_url().'vafpress/js/admin.js', array('jquery'), VP_PFUI_VERSION);
		wp_enqueue_style('vp-post-formats-ui', vp_pfui_base_url().'vafpress/css/admin.css', array(), VP_PFUI_VERSION, 'screen');
